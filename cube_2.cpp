...
//Camera TheCamera(Camera::LANDOBJECT);//这里是地面模式
//
...
//
	...
	//自动对法线矢量进行归一化处理确保顶点法线总是单位矢量，不受坐标变换的影响，这有助于提高渲染的精确性，但会增加处理器负担。
...
bool Display(float timeDelta)
{
	if (Device)
	{
		...
		D3DXMATRIX scale_, V_;

		D3DXMatrixScaling(&scale_, 0.3, 0.3, 0.3);
		Device->SetTransform(D3DTS_WORLD, &scale_);
